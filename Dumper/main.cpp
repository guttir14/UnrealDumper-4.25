...
private:
    ...
    static bool FindObjObjects(byte* start, byte* end) {
        static std::vector<byte> sigv[] = { { 0x89, 0x0D, 0x00, 0x00, 0x00, 0x00, 0x48, 0x8B, 0xDF, 0x48, 0x89, 0x5C, 0x24}, { 0x89, 0x1D, 0x00, 0x00, 0x00, 0x00, 0x85, 0xff } };
        for (auto& sig : sigv)
        {
        ...
        return ObjObjects.NumElements != 0;
    }
    static bool FindGlobalNames(byte* start, byte* end) {
        static std::vector<byte> sigv[] = { { 0x48, 0x8b, 0x3d, 0x00, 0x00, 0x00, 0x00, 0x48, 0x85, 0xff, 0x75, 0x3c }, { 0x48, 0x8B, 0x35, 0x00, 0x00, 0x00, 0x00, 0x41, 0x0f, 0xb7, 0xc4 } };
        for (auto& sig : sigv)
        {
